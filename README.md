...
透明・常時最前面のデスクトップ常駐コンシェルジュアプリ(Electron製)。画面上の気になった文字をクリックするだけで、Gemini APIが簡潔に解説してくれます。
...
5. 調べたい文字の上で**クリック**すると、その周辺をOCRで読み取り、Gemini APIに解説を依頼します。結果はクリックした位置の近くに吹き出しで表示され、1回調べると「調べるモード」は自動的に終了します。
