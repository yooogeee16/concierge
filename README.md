...
3. マスコットはつかんでドラッグすると好きな位置に移動できます(動かさずに離せば下記の通りクリックとして扱われます)。
...
7. 調べた語句と説明は**既定でローカルの辞書に自動登録されます**。不要な場合だけ、吹き出しの「辞書に登録済み(取り消す)」ボタンを押すと取り消せます。保存した内容は、マスコットを右クリック→「辞書を見る」から確認・削除できます。
8. 「調べるモード」中にマスコットを**もう一度クリック**すると「詳しく解説モード」に切り替わります(マスコットの光る色が緑になります)。段落やまとまった範囲をドラッグすると、専用ウィンドウに見出し・箇条書き・覚え方つきの解説ノートを表示します。ノートの中で扱った重要な語句も自動でいくつか辞書に登録され、ウィンドウ下部の一覧から不要なものだけ個別に取り消せます。
9. どちらのモードも `Esc` キーで終了できます。マスコットをさらにクリックすると、詳しく解説モード→アイドルの順に戻ります。
10. 辞書に何か登録していると、**1分に1回くらいの頻度**で、マスコットが登録語句について「〇〇って覚えてる?」と吹き出しで聞きに来ます(調べるモード/詳しく解説モード中は聞きに来ません)。「覚えてる!」「覚えてない…」ボタンで答えると閉じます。まだ出題していない語句を最優先にし、出題済みの中では「覚えてない」と答えた語句が優先的に(ただし「覚えてる」ものもたまに混ざるように)選ばれます。復習代わりにどうぞ。
...
右クリックメニューの「コードを解説する」から、コードファイル(JavaScript / TypeScript / Python / C / C++ / C#)をドラッグ&ドロップすると、
...
React・pandas・NumPy・Express・Electronの5つは、ライブラリ名だけでなく実際に使っている個々の関数/フック/メソッド(例: React なら `useState`・`useEffect`・`Suspense` など)まで図鑑として分けて集計します。それぞれ専用のセクションになっていて、検索するとヒットしたセクションだけが進捗バー付きで表示されるので、どのライブラリのどの項目を見つけたかが一目で分かります。それ以外のライブラリは「その他ライブラリ」としてまとめて表示されます。

さらにJavaScript・TypeScript・C++・C#は、言語そのものについても専用の図鑑があります(importの有無に関わらず、その言語のファイルなら常に集計対象)。

| 言語図鑑 | 集計する内容の例 |
| --- | --- |
| JavaScript | `Array.map`・`Object.keys`・`Promise.all`・オプショナルチェーンなど、言語組み込みの機能 |
| TypeScript | `interface`・ジェネリクス・`as`・`Partial<T>`などのユーティリティ型 |
| C++ | `vector`・`unique_ptr`・`make_shared`・`auto`・ラムダ式などSTL/言語機能 |
| C# | LINQ(`Select`・`Where`・`ToList`など)・プロパティ・`record`・`using`ブロックなど |
...
設定・辞書・図鑑のデータは `%APPDATA%/concierge/` に保存されており、インストーラーで新しいバージョンに更新してもそのまま引き継がれます。
...
├── preload-detail.js                 # 詳しく解説ウィンドウのpreload
├── preload-quiz.js                    # 辞書の復習クイズ吹き出しのpreload
├── src/
...
│   ├── codeObjectGroups.js               # ライブラリ別(React/pandas/NumPy/Express/Electron)・言語別(JS/TS/C++/C#)の詳細な図鑑と検出パターン
...
└── renderer/
    ...
    ├── code/                                 # コード解説ウィンドウ
    ├── codedex/                               # コード図鑑ウィンドウ
    ├── detail/                                 # 詳しく解説ウィンドウ(簡易Markdownレンダラー含む)
    └── quiz/                                    # 辞書の復習クイズ吹き出し
```
